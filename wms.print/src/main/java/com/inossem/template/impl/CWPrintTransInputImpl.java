...
/**  
* @Description  打印物料转储单
* @ClassName：CWPrintTransInputImpl   :
 *@Package com.inossem.template.impl 
 * @anthor：wyang
 * @date：2018年7月26日 下午1:52:52   
 * @版本：V1.0   
 */ 
public class CWPrintTransInputImpl extends PrintBaseCopy implements CopyTemplate {
	...
	public String mdPrint(List<Map> dataList, String type) throws Exception {
		...
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes())
				.getRequest();
		String filePath = request.getServletContext().getRealPath(Constant.TEMPLATE_LABEL_TRANSINPUT);
		// 读取模板
//		HSSFWorkbook hssInputExeclFile = new HSSFWorkbook(new FileInputStream(filePath));
		...
		// 二维码
		QRcodeUtil.zxingCodeCreate(Constant.ANDROID_TYPE_6+String.valueOf(header.get("stock_input_code")), 400, 400, imgPath, "png");
		...
		while (iterator.hasNext()) {
			...
			sheet.getRow(4 + count * 35).getCell(4).setCellValue("司机："+String.valueOf(header.get("driver")));
			sheet.getRow(4 + count * 35).getCell(8).setCellValue("单据编号："+String.valueOf(header.get("stock_input_code")));
			sheet.getRow(5 + count * 35).getCell(1).setCellValue("发出库房：" +String.valueOf( header.get("ware")));
			sheet.getRow(5 + count * 35).getCell(4).setCellValue("车辆："+String.valueOf(header.get("vehicle")));
			sheet.getRow(5 + count * 35).getCell(8).setCellValue("创建日期："+String.valueOf(header.get("create_time")));// 创建日期
			// sheet.getRow(6 + count *
			// 35).getCell(1).setCellValue(String.valueOf("接受工厂：" +
			// header.get("fty_input_name")));
			sheet.getRow(6 + count * 35).getCell(8).setCellValue("移动类型："+String.valueOf(header.get("move_type")));
			sheet.getRow(6 + count * 35).getCell(1)
					.setCellValue("备注说明：" + (header.get("remark") == null ? "" : String.valueOf(header.get("remark"))));
			...
			for (int i = 0; i < printItem.size(); i++) {
				...
				num += Double.parseDouble(String.valueOf(item.get(i).get("reqnum")));
