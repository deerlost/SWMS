...
public class CWPrintNewTaskImpl extends PrintBaseCopy implements CopyTemplate {
	...
	public String mdPrint(List<Map> dataList, String type) throws Exception {
		...
		if (Byte.parseByte(String.valueOf(header.get("receiptType"))) == DOWNTASK) {
			...
			filePath = request.getServletContext().getRealPath(Constant.TEMPLATE_LABEL_UPTASK);
		}
		// 读取模板
		...
		// 二维码
		QRcodeUtil.zxingCodeCreate(Constant.ANDROID_TYPE_6+String.valueOf(header.get("stock_task_code")), 400, 400, imgPath, "png");
		...
		while (iterator.hasNext()) {
			...
			if (Byte.parseByte(String.valueOf(header.get("receiptType"))) == DOWNTASK) {
				...
				sheet.getRow(4 + count * 35).getCell(6).setCellValue(String.valueOf(header.get("driver")));
				sheet.getRow(4 + count * 35).getCell(10).setCellValue(String.valueOf(header.get("stock_task_code")));// 作业单号
				sheet.getRow(5 + count * 35).getCell(1)
						.setCellValue(String.valueOf("发出库房：" + header.get("location_name")));
				sheet.getRow(5 + count * 35).getCell(6).setCellValue(String.valueOf(header.get("vehicle")));
				sheet.getRow(5 + count * 35).getCell(10).setCellValue(String.valueOf(header.get("create_time")));// 创建日期
				sheet.getRow(6 + count * 35).getCell(1)
						.setCellValue("交货单号：" + String.valueOf(header.get("receipt_code")));// 交货单号
				sheet.getRow(6 + count * 35).getCell(6).setCellValue(String.valueOf(header.get("receive_name")));// 客户名称
				sheet.getRow(6 + count * 35).getCell(10).setCellValue(String.valueOf(header.get("receipt_type_name")));// 单据类型
				sheet.getRow(7 + count * 35).getCell(1)
						.setCellValue(String.valueOf("备注说明：" + header.get("headremark")));
			} else {
				sheet.getRow(4 + count * 35).getCell(1).setCellValue(String.valueOf("工厂：" + header.get("fty_name")));
				sheet.getRow(4 + count * 35).getCell(5).setCellValue(String.valueOf(header.get("driver")));
				sheet.getRow(4 + count * 35).getCell(9).setCellValue("作业单：  "+String.valueOf(header.get("stock_task_code")));// 作业单号
				sheet.getRow(5 + count * 35).getCell(1)
						.setCellValue(String.valueOf("库房：" + header.get("location_name")));
				sheet.getRow(5 + count * 35).getCell(5).setCellValue(String.valueOf(header.get("vehicle")));
				sheet.getRow(5 + count * 35).getCell(9).setCellValue("创建日期："+String.valueOf(header.get("create_time")));// 创建日期
				sheet.getRow(6 + count * 35).getCell(1)
						.setCellValue("业务单号：" + String.valueOf(header.get("refer_receipt_code")));// 业务单号
				sheet.getRow(6 + count * 35).getCell(9).setCellValue("单据类型："+String.valueOf(header.get("receipt_type_name")));// 单据类型
				sheet.getRow(7 + count * 35).getCell(1)
						.setCellValue(String.valueOf("备注说明：" + header.get("headremark")));
			...
			for (int i = 0; i < printItem.size(); i++) {
				...
				sheet.getRow(10 + i + count * 35).getCell(4).setCellValue(String.valueOf(item.get(i).get("reqnum")));
				reqnum += Double.parseDouble(String.valueOf(item.get(i).get("reqnum")));
				sheet.getRow(10 + i + count * 35).getCell(5).setCellValue(String.valueOf(item.get(i).get("qty")));
				qty += Double.parseDouble(String.valueOf(item.get(i).get("qty")));
				sheet.getRow(10 + i + count * 35).getCell(6)
						.setCellValue(String.valueOf(item.get(i).get("production_batch")));
				sheet.getRow(10 + i + count * 35).getCell(7).setCellValue(String.valueOf(item.get(i).get("erp_batch")));
				sheet.getRow(10 + i + count * 35).getCell(8).setCellValue(String.valueOf(item.get(i).get("num")));
				sheet.getRow(10 + i + count * 35).getCell(9).setCellValue(String.valueOf(item.get(i).get("position")));
				sheet.getRow(10 + i + count * 35).getCell(10)
						.setCellValue(String.valueOf(item.get(i).get("itemremark")));
			}
			sheet.getRow(15 + count * 35).getCell(1).setCellValue("合计");
			sheet.getRow(15 + count * 35).getCell(4).setCellValue(reqnum);
			sheet.getRow(15 + count * 35).getCell(5).setCellValue(qty);

			sheet.getRow(17 + count * 35).getCell(2).setCellValue(String.valueOf("理货员：" + header.get("tally_clerk")));// 理货员
			sheet.getRow(17 + count * 35).getCell(4)
					.setCellValue(String.valueOf("叉车工：" + header.get("forklift_worker")));// 叉车工
			sheet.getRow(17 + count * 35).getCell(8).setCellValue(String.valueOf("搬运工：" + header.get("remover")));// 搬运工
		...
		// 设置打印区域
		wb.setPrintArea(0, 0, 10, 0, sheet.getLastRowNum()-1);
