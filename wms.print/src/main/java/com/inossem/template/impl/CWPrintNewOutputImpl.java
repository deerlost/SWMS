...
public class CWPrintNewOutputImpl extends PrintBaseCopy implements CopyTemplate {
	...
	public String mdPrint(List<Map> dataList, String type) throws Exception {
		...
		// 二维码
		QRcodeUtil.zxingCodeCreate(Constant.ANDROID_TYPE_6+String.valueOf(header.get("allocate_cargo_code")), 400, 400, imgPath, "png");
		...
		while (iterator.hasNext()) {
			...
			sheet.getRow(4 + count * 35).getCell(10).setCellValue(String.valueOf(header.get("allocate_cargo_code")));// 作业单号
			sheet.getRow(5 + count * 35).getCell(1).setCellValue(String.valueOf("发出库房：" + header.get("ware")));
			...
			sheet.getRow(6 + count * 35).getCell(1).setCellValue(String.valueOf("交货单号：" + header.get("refer_receipt_code")));
			sheet.getRow(6 + count * 35).getCell(4).setCellValue(String.valueOf("客户名称：" + header.get("receive_name")));
			sheet.getRow(7 + count * 35).getCell(1).setCellValue(String.valueOf("备注说明：" + header.get("head_remark")));
			...
			for (int i = 0; i < printItem.size(); i++) {
				...
				sheet.getRow(10 + i + count * 35).getCell(5)
						.setCellValue(String.valueOf(item.get(i).get("output_qty")));
				qty += Double.parseDouble(String.valueOf(item.get(i).get("output_qty")));
				...
				sheet.getRow(10 + i + count * 35).getCell(9)
						.setCellValue(String.valueOf(item.get(i).get("position")));
				sheet.getRow(10 + i + count * 35).getCell(10).setCellValue(String.valueOf(item.get(i).get("item_remark")));
			}
			...
			sheet.getRow(17 + count * 35).getCell(7).setCellValue("打印日期："+String.valueOf(header.get("print_date")));// 日期
