package com.inossem.wms.portable.filter;
...
public class ModifyJsonKeyFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		// 判断请求的ContentType是否为application/json;charset=utf-8
    	if(hrequest.getContentType() != null && ("application/json; charset=utf-8".equalsIgnoreCase(hrequest.getContentType().trim())
    					...
    					|| "application/json".equalsIgnoreCase(hrequest.getContentType().trim()))) {
    		// 
    		...
    		if(out.length() > 0 && out.trim().startsWith("{") && out.endsWith("}")) {
    			...
    			// 将json字符结构的key转换为snake形式,且JSON值全部转换为字符串
    			Object newOut = UtilJSONConvert.convertForApp(out);
