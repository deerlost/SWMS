...
function detail_table_tap(){
    var url1=uriapi+"/conf/warring/select_detail?search_type=1"
    var detailsArray = [];
    $("#select_detail").html("");
...
function detail_table_bottom(wh_id){
    var url1=uriapi+"/conf/warring/select_detail_mat?search_type=1&wh_id="+wh_id
    var detailsArray = [];
    ...
    ajax(url1,"get",null,function(data){
        ...
        $.each(detailsArray,function(i,item){
            str+='<li>';
            str+='<span class="ol2" style="text-align: left">'+(i+1)+'</span>';
