...
import com.inossem.wms.service.biz.IInputUrgentProduceService;
...
@Service("inputUrgentProduceTransportService")
@Transactional
public class InputUrgentProduceServiceImpl implements IInputUrgentProduceService {
...
//	}
	...
	@Override
	public List<Map<String, Object>> selectAllErpList() {
		//return bizInputProduceHeadDao.selectAllErpList();
	...
	public List<Map<String, Object>> selectPackageTypeList(String mat_id) {
		return bizInputProduceHeadDao.selectPackageTypeList(mat_id);
	}
...
//		return bizInputProduceHeadDao.selectLocationList(fty_id);
//	}
	...
	public Map<String,Object> insertProduction(CurrentUser user, JSONObject json) throws Exception {
		...
		record.setReceiptType(EnumReceiptType.STOCK_INPUT_PRODUCTION_URGENT.getValue());
		record.setStatus(EnumReceiptStatus.RECEIPT_SUBMIT.getValue());
...
//			itdocitemtest.setBATCH("P0000");
	...
	@Override
	public void modifyNum(BizStockInputHead bizStockInputHead,CurrentUser user, JSONObject json,JSONArray batchArray) throws Exception {
		...
		for(int i=0;i<myJsonArray.size();i++) {						
			...
			stockBatchMap.put("isUrgent", EnumIsUrgent.URGENT.getValue());
			...
			if (dicPackageTypeDao.selectByPrimaryKey(Integer.valueOf(obj.getString("package_type_id"))).getSnUsed().equals(EnumMatStoreType.USEING.getValue())) {
				...
				for(int j=0;j<pkgArray.size();j++) {				
					...
					for(int u=0;u<pkgPositionList.size();u++) {
						...
						stockPositionIn.setInputDate(new Date());
						stockPositionIn.setIsUrgent(EnumIsUrgent.URGENT.getValue());
	...
	public void modifyNum2(BizStockInputHead bizStockInputHead,CurrentUser user, JSONObject json,JSONArray batchArray,JSONArray itemArray) throws Exception {
		...
		for(int i=0;i<myJsonArray.size();i++) {						
			...
			stockBatchMap.put("status", EnumStockStatus.STOCK_BATCH_STATUS_UNRESTRICTED.getValue());
			stockBatchMap.put("createUserId", user.getUserId());
			stockBatchMap.put("debitCredit", "H");
			stockBatchMap.put("qty", obj.get("input_stock_num"));
			stockBatchMap.put("isUrgent", EnumIsUrgent.URGENT.getValue());
			stockBatchMap.put("stockTypeId",EnumStockType.STOCK_TYPE_ERP.getValue());
			commonService.modifyStockBatchOnSyn(stockBatchMap);
			...
			if (dicPackageTypeDao.selectByPrimaryKey(Integer.valueOf(id)).getSnUsed().equals(EnumMatStoreType.USEING.getValue())) {
				...
				for(int j=0;j<pkgArray.size();j++) {				
					...
					for(int u=0;u<pkgPositionList.size();u++) {
						...
						stockPositionUrgent.setWhId(whId);
						stockPositionUrgent.setDebitOrCredit(EnumDebitCredit.CREDIT_H_SUBTRACT.getName());
						stockPositionUrgent.setAreaId(sourceAreaId);
						stockPositionUrgent.setPositionId(sourcePositionId);
						stockPositionUrgent.setMatId(Integer.valueOf(json.get("mat_id").toString()));
						stockPositionUrgent.setFtyId(Integer.valueOf(json.get("fty_id").toString()));
						stockPositionUrgent.setLocationId(Integer.valueOf(obj.get("location_id").toString()));
						stockPositionUrgent.setBatch(new Long(batchArray.get(i).toString()));
						stockPositionUrgent.setPackageTypeId(Integer.valueOf(id.toString()));
						stockPositionUrgent.setInputDate(new Date());
						stockPositionUrgent.setQty(new BigDecimal(pkgPositionList.get(u).get("package_standard").toString()));
						stockPositionUrgent.setUnitId(Integer.valueOf(json.get("unit_id").toString()));
						stockPositionUrgent.setInputDate(new Date());
						stockPositionUrgent.setPackageId(Integer.valueOf(pkgPositionList.get(u).get("package_id").toString()));
						stockPositionUrgent.setPalletId(Integer.valueOf(pkgPositionList.get(u).get("pallet_id").toString()));
						...
						stockPositionUrgent.setStockTypeId(EnumStockType.STOCK_TYPE_ERP.getValue());
						commonService.modifyStockPositionOnSyn(stockPositionUrgent);	
			...
			stockPositionUrgent.setPackageTypeId(Integer.valueOf(id.toString()));
			stockPositionUrgent.setInputDate(new Date());
			stockPositionUrgent.setQty(new BigDecimal(obj.get("input_stock_num").toString()));
			stockPositionUrgent.setUnitId(Integer.valueOf(json.get("unit_id").toString()));
			stockPositionUrgent.setInputDate(new Date());
		...
		bih.setPurchaseOrderCode(bizStockInputHead.getPurchaseOrderCode());
...
//			taskReqItem.setReferReceiptId(bizStockInputTransportHead.getStockInputId());
	...
	@Override
	public void updateWriteOffStatus(ErpReturn er,JSONArray item_id, int stock_input_id) {
		...
		for(int z=0;z<list.size();z++) {
			...
			if(!dicLocationDao.selectByPrimaryKey(Integer.valueOf(itemList.get("location_id").toString())).getLocationCode().equals("6006") ) {
				...
				for(int k=0;k<mescodeList.size();k++) {
					...
					String id=itemList.getString("package_standard").toString();
					if(!map.containsKey(id+
							mesObj.get("mes_code").toString().trim()+
							itemList.get("erp_batch").toString()+
							itemList.get("location_id").toString()								
		...
		for(int i=0;i<list.size();i++) {	
			...
			if(!dicLocationDao.selectByPrimaryKey(Integer.valueOf(itemList.get("location_id").toString())).getLocationCode().equals("6006")) {
				...
				for(int j=0;j<mescodeList.size();j++) {	
					...
					String rel=stockInputHeadDao.getUnitRelMesAndWms(param);
					String id=itemList.getString("package_standard").toString();
					dto.setWgtPerPack(Double.parseDouble(dicPackageTypeDao.selectByPrimaryKey(Integer.valueOf(itemList.getString("package_type_id").toString())).getPackageStandard().toString()));
					dto.setAmnt(new BigDecimal(mesObj.get("input_stock_num").toString()).divide(new BigDecimal(itemList.get("package_standard").toString()),0, RoundingMode.UP).intValue());
					dto.setWgt(new BigDecimal(mesObj.get("input_stock_num").toString()).divide(new BigDecimal(rel)).doubleValue());
					dto.setSrBch(UtilObject.getStringOrEmpty(mesObj.get("mes_bch")).equals("")?"00000":UtilObject.getStringOrEmpty(mesObj.get("mes_bch")));
					dto.setTgBch(itemList.get("erp_batch").toString());
...
//		}
	...
	@Override
	public List<Map<String, Object>> selectAllPackageTypeList() {
		//return bizInputProduceHeadDao.selectAllPackageTypeList();
		return new ArrayList<Map<String,Object>>();
	}
	...
	@Override
	public List<Map<String, Object>> selectFactoryList(CurrentUser cuser) {
		List<Map<String, Object>> list= bizInputProduceHeadDao.selectFactoryList();
		for(int i=0;i<list.size();i++) {
			Map<String, Object> param=new HashMap<String,Object>();
			param.put("fty_id", Integer.valueOf(list.get(i).get("fty_id").toString()));
			param.put("userId", cuser.getUserId());
			list.get(i).put("location_list", bizInputProduceHeadDao.selectLocationList(param));
		}
		return list;
	}
	...
	@Override
	public Map<String, Object> select_mat_info(String mat_code) {
		return bizInputProduceHeadDao.select_mat_info(mat_code);
	}



	@Override
	public void delete(String stock_input_id,CurrentUser user) throws Exception {
		BizStockInputHead bizStockInputHead=new BizStockInputHead();
		bizStockInputHead.setStockInputId(Integer.valueOf(stock_input_id));
		bizStockInputHead.setIsDelete((byte) 1);
		bizStockInputHeadMapper.updateByPrimaryKeySelective(bizStockInputHead);
		
		
		bizInputProduceHeadDao.updateItemStatus(stock_input_id);
		bizInputProduceHeadDao.updateStatusByInputId(stock_input_id);
		
		BizStockInputHead head=bizStockInputHeadMapper.selectByPrimaryKey(Integer.valueOf(stock_input_id));
		List<Map<String,Object>> myJsonArray=bizInputProduceHeadDao.selectByStockTransportId(Integer.valueOf(stock_input_id));
		
		
		for(int i=0;i<myJsonArray.size();i++) {						
			JSONObject obj =JSONObject.fromObject(myJsonArray.get(i));			
			Map<String, Object> stockBatchMap=new HashMap<String,Object>();
			stockBatchMap.put("matId", obj.get("mat_id"));
			stockBatchMap.put("locationId",obj.get("location_id"));
			stockBatchMap.put("batch", obj.get("batch"));
			stockBatchMap.put("status", EnumStockStatus.STOCK_BATCH_STATUS_UNRESTRICTED.getValue());
			stockBatchMap.put("createUserId", user.getUserId());
			stockBatchMap.put("debitCredit", EnumDebitCredit.CREDIT_H_SUBTRACT.getName());
			stockBatchMap.put("qty", obj.get("qty"));
			stockBatchMap.put("isUrgent", EnumIsUrgent.URGENT.getValue());
			stockBatchMap.put("stockTypeId",EnumStockType.STOCK_TYPE_ERP.getValue());
			commonService.modifyStockBatch(stockBatchMap);
			...
			if (dicPackageTypeDao.selectByPrimaryKey(Integer.valueOf(obj.getString("package_type_id"))).getSnUsed().equals(EnumMatStoreType.USEING.getValue())) {
				Map<String, Object> param=new HashMap<String, Object>();
				param.put("stock_input_id", stock_input_id.toString());
				param.put("rid", i+1);
				List<Map<String, Object>> packList=bizInputProduceHeadDao.getPackageOperationlist(param);					
				for(int j=0;j<packList.size();j++) {				
					List<Map<String,Object>> pkgPositionList=bizInputProduceHeadDao.getPkgPosition(packList.get(j).get("package_id").toString());			
					for(int u=0;u<pkgPositionList.size();u++) {
						...
						key.setPackageId(Integer.valueOf(pkgPositionList.get(u).get("package_id").toString()));// 包
						key.setMatId(Integer.valueOf(obj.get("mat_id").toString())); // 物料id,
						key.setLocationId(Integer.valueOf(obj.get("location_id").toString()));// 库存地点id
						key.setBatch(new Long(obj.get("batch").toString())); // 批号id
						key.setStatus(EnumStockStatus.STOCK_BATCH_STATUS_UNRESTRICTED.getValue());
						// 紧急查询
						key.setIsUrgent(EnumIsUrgent.URGENT.getValue());
						...
						if(stockPosition.getQty().compareTo(UtilObject.getBigDecimalOrZero(pkgPositionList.get(u).get("package_standard"))) == -1) {
						...
						stockPositionIn.setAreaId(sourceAreaId);
						stockPositionIn.setPositionId(sourcePositionId);
						stockPositionIn.setMatId(Integer.valueOf(obj.get("mat_id").toString()));
						stockPositionIn.setFtyId(Integer.valueOf(obj.get("fty_id").toString()));
						stockPositionIn.setLocationId(Integer.valueOf(obj.get("location_id").toString()));
						stockPositionIn.setBatch(new Long(obj.get("batch").toString()));
						stockPositionIn.setPackageTypeId(Integer.valueOf(obj.get("package_type_id").toString()));
						stockPositionIn.setInputDate(new Date());
						stockPositionIn.setQty(new BigDecimal(pkgPositionList.get(u).get("package_standard").toString()));
						stockPositionIn.setUnitId(Integer.valueOf(obj.get("unit_id").toString()));
						stockPositionIn.setInputDate(new Date());
						stockPositionIn.setIsUrgent(EnumIsUrgent.URGENT.getValue());
						stockPositionIn.setStockTypeId(EnumStockType.STOCK_TYPE_ERP.getValue());
						stockPositionIn.setPalletId(Integer.valueOf(pkgPositionList.get(u).get("pallet_id").toString()));					
						...
						record.setAreaId(sourceAreaId);
						record.setBatch(new Long(obj.get("batch").toString()));
						record.setBusinessType((byte) 1);
						record.setCreateTime(new Date());
						record.setCreateUser(user.getUserId());
						record.setDebitCredit(EnumDebitCredit.CREDIT_H_SUBTRACT.getName());
						record.setFtyId(Integer.valueOf(obj.get("fty_id").toString()));
						record.setLocationId(Integer.valueOf(obj.get("location_id").toString()));
						record.setMatId(Integer.valueOf(obj.get("mat_id").toString()));
						record.setModifyTime(new Date());
						record.setModifyUser(user.getUserId());
						record.setQty(new BigDecimal(pkgPositionList.get(u).get("package_standard").toString()));
						record.setPackageId(Integer.valueOf(pkgPositionList.get(u).get("package_id").toString()));
						record.setPalletId(Integer.valueOf(pkgPositionList.get(u).get("pallet_id").toString()));
						record.setPositionId(sourcePositionId);
						record.setReceiptType(head.getReceiptType());
						record.setReferReceiptCode(head.getStockInputCode());
						record.setReferReceiptId(head.getStockInputId());
						record.setReferReceiptPid(whId);
						record.setReferReceiptRid(i+1);
						record.setSynMes(true);
						record.setSynSap(true);
			...
			if(stockPosition!=null){
				if(stockPosition.getQty().compareTo(UtilObject.getBigDecimalOrZero(obj.get("qty").toString())) == -1) {
					throw new WMSException("库存不足");
				}
			}else{
				throw new WMSException("库存不足");
			...
			stockPosition.setAreaId(sourceAreaId);
			stockPosition.setPositionId(sourcePositionId);
			stockPosition.setMatId(Integer.valueOf(obj.get("mat_id").toString()));
			stockPosition.setFtyId(Integer.valueOf(obj.get("fty_id").toString()));
			stockPosition.setLocationId(Integer.valueOf(obj.get("location_id").toString()));
			stockPosition.setBatch(new Long(obj.get("batch").toString()));
			stockPosition.setPackageTypeId(Integer.valueOf(obj.get("package_type_id").toString()));
			stockPosition.setInputDate(new Date());
			stockPosition.setQty(new BigDecimal(obj.get("qty").toString()));
			stockPosition.setUnitId(Integer.valueOf(obj.get("unit_id").toString()));
			stockPosition.setInputDate(new Date());
			stockPosition.setPackageId(0);
			stockPosition.setPalletId(0);
			stockPosition.setStatus(EnumStockStatus.STOCK_BATCH_STATUS_UNRESTRICTED.getValue());
			...
			record.setCreateTime(new Date());
			record.setQty(new BigDecimal(obj.get("qty").toString()));				
			record.setDebitCredit(EnumDebitCredit.CREDIT_H_SUBTRACT.getName());
			record.setFtyId(Integer.valueOf(obj.get("fty_id").toString()));
			record.setLocationId(Integer.valueOf(obj.get("location_id").toString()));
			record.setMatId(Integer.valueOf(obj.get("mat_id").toString()));
