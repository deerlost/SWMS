...
@Service("inputVirtualProduceTransportService")
@Transactional
public class InputVirtualProduceServiceImpl implements IInputVirtualProduceService {
...
//	}
	...
	public Map<String,Object> insertProduction(CurrentUser user, JSONObject json) throws Exception {
		...
		record.setReceiptType(EnumReceiptType.STOCK_INPUT_VIRTUAL.getValue());
...
//			}
			...
			for(int j=0;j<pkgArray.size();j++) {
				...
				head.setBeforeOrderType(EnumReceiptType.STOCK_INPUT_VIRTUAL.getValue());
...
//			List<Map<String, Object>> packList=bizInputProduceHeadDao.getPackageOperationlist(list.get(i).get("package_type_item_id").toString());
...
//			itdocitemtest.setBATCH("P0000");	
	...
	public void modifyNum(BizStockInputHead bizStockInputHead,CurrentUser user, JSONObject json,JSONArray batchArray,JSONArray itemArray) throws Exception {
		...
		for(int i=0;i<myJsonArray.size();i++) {						
			...
			stockBatchMap.put("stockTypeId",EnumStockType.STOCK_TYPE_VIRTUAL.getValue());
			...
			if (dicPackageTypeDao.selectByPrimaryKey(Integer.valueOf(obj.getString("package_type_id"))).getSnUsed().equals(EnumMatStoreType.USEING.getValue())) {
				...
				for(int j=0;j<pkgArray.size();j++) {				
					...
					for(int u=0;u<pkgPositionList.size();u++) {
						...
						stockPositionIn.setInputDate(new Date());
						stockPositionIn.setStockTypeId(EnumStockType.STOCK_TYPE_VIRTUAL.getValue());
...
//			}	
	...
	@Override
	public List<String>  selectTranspotStatus(String stock_input_id) {
		return bizInputProduceHeadDao.selectTranspotStatus(stock_input_id);
	}
	...
	public String selectIsWriteOff(int stock_input) {
		...
		for(Map<String,Object> map:list) {
			...
			param.put("stock_type_id",EnumStockType.STOCK_TYPE_VIRTUAL.getValue());
