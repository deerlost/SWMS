...
@Controller
public class UrgentProductionTransportControllerCW {

	private static final Logger logger = LoggerFactory.getLogger(UrgentProductionTransportControllerCW.class);
	@Autowired
	...
	// 显示当前登录用户的库存地点
	@RequestMapping(value = "/biz/urgent/product/transport/base_info", method = RequestMethod.GET, produces = "application/json;charset=UTF-8")
	public @ResponseBody JSONObject getFtyLocationForUser(CurrentUser cUser) {
		...
		try {
			ftyListIn = commonService.listFtyLocationForUser(userId,"");
	...
	// 显示当前登录用户的库存地点
	@RequestMapping(value = "/biz/urgent/product/transport/material_list", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
...
//			}
	...
	// 提交生产转运单
	@RequestMapping(value = "/biz/urgent/product/transport/submit", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	public @ResponseBody JSONObject submit(@RequestBody JSONObject json, CurrentUser user) {
		...
		try {
			...
			transportHead.setReceiptType(EnumReceiptType.STOCK_TRANSPORT_PRODUCTION_URGENT.getValue());
	...
	@RequestMapping(value = {
			"/biz/urgent/product/transport/check_code_type" }, method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	 ...
	 * @return
	 */
	@RequestMapping(value = "/biz/urgent/product/transport/head_list", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	public @ResponseBody Object getHeadList(CurrentUser cuser, @RequestBody JSONObject json) {
		...
		try {
			...
			map.put("receiptType", EnumReceiptType.STOCK_TRANSPORT_PRODUCTION_URGENT.getValue());
			...
			if (shList != null && shList.size() > 0) {
				...
				for (BizStockTransportHeadCw sh : shList) {
					...
					itemMap.put("location_code", UtilString.getStringOrEmptyForObject(sh.getLocationCode()));
					...
					itemMap.put("status_name", UtilString.getStringOrEmptyForObject(
							EnumReceiptStatus.getNameByValue(sh.getStatus())));
	...
	@RequestMapping(value = "/biz/urgent/product/transport/details/{stock_transport_id}", method = RequestMethod.GET, produces = "application/json;charset=UTF-8")
		...
		try {
			if (stockTransportId > 0) {
				...
				if (getList != null && getList.size() > 0) {
					...
					List<BizReceiptAttachmentVo> fileList = fileService.getReceiptAttachments(body.getInt("stock_transport_id"),
							UtilObject.getByteOrNull(body.get("receipt_type")));
	...
	@RequestMapping(value = "/biz/urgent/product/transport/delete/{stock_transport_id}", method = RequestMethod.DELETE, produces = "application/json;charset=UTF-8")
	public @ResponseBody JSONObject delete(@PathVariable("stock_transport_id") int stockTransportId,
		...
		JSONObject body = new JSONObject();
		try {
			productionTransportService.deleteById(stockTransportId, EnumReceiptType.STOCK_TRANSPORT_PRODUCTION_URGENT.getValue());
