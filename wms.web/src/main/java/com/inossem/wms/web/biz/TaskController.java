...
public class TaskController {
	...
	@Autowired
	private IPkgOperationService packageService;
	 ...
	 * // 仓库整理--基于仓位调整提交
	 * 
	 * @RequestMapping(value = "/warehouse/position_submit/old", method =
	 * RequestMethod.POST, produces = "application/json;charset=UTF-8")
	 * public @ResponseBody JSONObject positionSubmitOld(@RequestBody JSONObject
	 * json, CurrentUser cUser) { JSONObject body = new JSONObject(); int
	 * error_code = EnumErrorCode.ERROR_CODE_FAILURE.getValue(); int pageIndex =
	 * EnumPage.PAGE_INDEX.getValue(); int pageSize =
	 * EnumPage.PAGE_SIZE.getValue(); boolean returnStatus = false; String
	 * errorString = ""; int total = 0; try { int whId = json.getInt("wh_id");
	 * int sourceAreaId = json.getInt("s_area_id"); int sourcePositionId =
	 * json.getInt("s_position_id"); int targetAreaId =
	 * json.getInt("t_area_id"); int targetPositionId =
	 * json.getInt("t_position_id"); int ftyId = json.getInt("fty_id"); int
	 * locationId = json.getInt("location_id"); String remark =
	 ...
	 * SimpleDateFormat sdf2 = new SimpleDateFormat("HH:mm:ss"); // Date date =
	 * new Date();
	 * 
	 * BizStockTaskHead bizStockTaskHead = new BizStockTaskHead();
	 * bizStockTaskHead.setWhId(whId); bizStockTaskHead.setMoveTypeId(0);
	 ...
	 * bizStockTaskHead.setReceiptType(EnumReceiptType.
	 * STOCK_TASK_POSITION_CLEANUP.getValue());
	 * bizStockTaskHead.setStockTaskReqId(0);
	 * bizStockTaskHead.setRemark(remark);
	 * 
	 * List<Map<String, String>> checkOutMaps = new ArrayList<Map<String,
	 * String>>(); List<Map<String, String>> checkInMaps = new
	 * ArrayList<Map<String, String>>();
	 * 
	 * HashMap<String, String> checkOutMap = new HashMap<String, String>();
	 * checkOutMap.put("position_id", sourcePositionId + "");
	 * checkOutMaps.add(checkOutMap); HashMap<String, String> checkInMap = new
	 * HashMap<String, String>(); checkInMap.put("position_id", targetPositionId
	 * + ""); checkInMaps.add(checkInMap);
	 * 
	 * FreezeCheck fOutCheck =
	 * commonService.checkWhPosAndStockLoc(checkOutMaps); FreezeCheck fInCheck =
	 * commonService.checkWhPosAndStockLoc(checkInMaps); if (fOutCheck != null
	 * && !fOutCheck.isLockedOutput() && fInCheck != null &&
	 * !fInCheck.isLockedInput()) {
	 * taskServiceImpl.positionCleanupOld(bizStockTaskHead, whId, sourceAreaId,
	 * sourcePositionId, targetAreaId, targetPositionId); returnStatus = true;
	 * error_code = EnumErrorCode.ERROR_CODE_SUCESS.getValue();
	 * body.put("position_cleanup", true);
	 * 
	 * } else { body.put("position_cleanup", false); if (fOutCheck != null &&
	 * StringUtils.hasText(fOutCheck.getResultMsgOutput())) { errorString +=
	 * fOutCheck.getResultMsgOutput(); } if (fInCheck != null &&
	 * StringUtils.hasText(fInCheck.getResultMsgOutput())) { errorString +=
	 * fInCheck.getResultMsgOutput(); } }
	 * 
	 * } catch (NegativeInventoryException e) { logger.error("", e); errorString
	 * = "发出仓位与接受仓位不能相同"; } catch (Exception e) { logger.error("基于仓位提交", e); }
	 * 
	 * return UtilResult.getResultToJson(returnStatus, error_code, errorString,
	 * pageIndex, pageSize, total, body); }
	 */
	...
	public @ResponseBody JSONObject positionSubmit(@RequestBody JSONObject json, CurrentUser cUser) {
		...
		try {
			...
			int sourceAreaId = json.getInt("s_area_id");
			int sourcePositionId = json.getInt("s_position_id");
			...
			if (fOutCheck != null && !fOutCheck.isLockedOutput() && fInCheck != null && !fInCheck.isLockedInput()) {
				...
				body.put("position_cleanup", false);
		...
		return UtilResult.getResultToJson(returnStatus, error_code, errorString, body);
	}

	// 仓库整理--基于物料调整提交
	// @RequestMapping(value = "/warehouse/material_submit/old", method =
	// RequestMethod.POST, produces = "application/json;charset=UTF-8")
	// public @ResponseBody JSONObject materialSubmitOld(@RequestBody JSONObject
	// json, CurrentUser cUser) {
	// JSONObject body = new JSONObject();
	// int error_code = EnumErrorCode.ERROR_CODE_FAILURE.getValue();
	// int pageIndex = EnumPage.PAGE_INDEX.getValue();
	// int pageSize = EnumPage.PAGE_SIZE.getValue();
	...
	// bizStockTaskHead.setRemark(remark);
	//
	// List<BizStockTaskItem> list = new ArrayList<BizStockTaskItem>();
	// List<Map<String, String>> checkOutMaps = new ArrayList<Map<String,
	// String>>();
	// List<Map<String, String>> checkInMaps = new ArrayList<Map<String,
	// String>>();
	//
	...
	// int snId = material.getInt("sn_id");// sn库存
	// int matId = material.getInt("mat_id");// 物料
	// int sourceAareaId = material.getInt("s_area_id");// 源存储区
	// int sourcePositionIid = material.getInt("s_position_id");// 源仓位
	//
	...
	//
	// List<StockPositionVo> materialInfoList =
	// taskServiceImpl.getMaterialInPosition(stockPosition);
	// StockPositionVo stockPositionVo = materialInfoList.get(0);
	// JSONArray targetCodeArray =
	// material.getJSONArray("target_position_list");// 目标-存储区code-仓位code
	// for (Object innerObject : targetCodeArray) {
	// BizStockTaskItem bizStockTaskItem = new BizStockTaskItem();
	// bizStockTaskItem.setStockPositionId(stockPositionId);
	// bizStockTaskItem.setBatch(stockPositionVo.getBatch());
	// bizStockTaskItem.setFtyId(ftyId);
	// bizStockTaskItem.setIsDelete((byte) 0);
	// bizStockTaskItem.setWhId(whId);
	// bizStockTaskItem.setLocationId(locationId);
	// bizStockTaskItem.setMatId(matId);
	// bizStockTaskItem.setSourceAreaId(sourceAareaId);
	// bizStockTaskItem.setSourcePositionId(sourcePositionIid);
	// bizStockTaskItem.setSourceStockType(0);
	// bizStockTaskItem.setSpecStock(UtilString.getStringOrEmptyForObject(stockPositionVo.getSpecStock()));
	// bizStockTaskItem
	// .setSpecStockCode(UtilString.getStringOrEmptyForObject(stockPositionVo.getSpecStockCode()));
	// bizStockTaskItem
	// .setSpecStockName(UtilString.getStringOrEmptyForObject(stockPositionVo.getSpecStockName()));
	// bizStockTaskItem.setTargetStockType(0);
	// bizStockTaskItem.setUnitId(stockPositionVo.getUnitId());
	// bizStockTaskItem.setUnitWeight(stockPositionVo.getUnitWeight());
	//
	// JSONObject targetPositionJson = (JSONObject) innerObject;
	// BigDecimal qty = new BigDecimal(targetPositionJson.getString("qty"));//
	// 调整数量
	// String targetAreaPositionCode =
	// targetPositionJson.getString("t_area_position_code"); // 源-存储区code-仓位code
	// String[] targetAreaPositionCodeAry = targetAreaPositionCode.split("-",
	// 2);
	// String targetAreaCode = targetAreaPositionCodeAry[0];// 存储区code
	// int targetAreaId = dictionaryService.getAreaIdByWhCodeAndAreaCode(whCode,
	// targetAreaCode);
	...
	// DicWarehousePosition targetPosition =
	// taskServiceImpl.selectDicWarehousePositionByKey(targetKey);
	// int targetPositionId = targetPosition.getPositionId();
	//
	// bizStockTaskItem.setQty(qty);
	// bizStockTaskItem.setTargetAreaId(targetAreaId);
	// bizStockTaskItem.setTargetPositionId(targetPositionId);
	// list.add(bizStockTaskItem);
	//
	// HashMap<String, String> checkOutMap = new HashMap<String, String>();
	// checkOutMap.put("position_id", sourcePositionIid + "");
	// checkOutMaps.add(checkOutMap);
	// HashMap<String, String> checkInMap = new HashMap<String, String>();
	// checkInMap.put("position_id", targetPositionId + "");
	// checkInMaps.add(checkInMap);
	...
	//
	// FreezeCheck fOutCheck =
	// commonService.checkWhPosAndStockLoc(checkOutMaps);
	// FreezeCheck fInCheck = commonService.checkWhPosAndStockLoc(checkInMaps);
	//
	// if (fOutCheck != null && !fOutCheck.isLockedOutput() && fInCheck != null
	// && !fInCheck.isLockedInput()) {
	// taskServiceImpl.materialCleanupOld(bizStockTaskHead, list);
	// returnStatus = true;
	// error_code = EnumErrorCode.ERROR_CODE_SUCESS.getValue();
	// } else {
	// if (fOutCheck != null &&
	// StringUtils.hasText(fOutCheck.getResultMsgOutput())) {
	// errorString += fOutCheck.getResultMsgOutput();
	// }
	// if (fInCheck != null &&
	// StringUtils.hasText(fInCheck.getResultMsgOutput())) {
	// errorString += fInCheck.getResultMsgOutput();
	// }
	// }
	// } catch (Exception e) {
	...
	public @ResponseBody JSONObject materialSubmit(@RequestBody JSONObject json, CurrentUser cUser) {
		...
		try {
			...
			for (Object object : materialItem) {
				...
				// 后续 可能会有目标包,目标托盘
				// int packageId = material.getInt("package_id");// 包
				// int palletId = material.getInt("pallet_id");// 托盘
				// int locationId = material.getInt("location_id");// 库存地点
		...
		try {
			...
			if (list != null && list.size() > 0) {
				...
				for (StockPositionVo stockPositionVo : list) {
					...
					innerObject.put("batch_qty", stockPositionVo.getQty());// 批次数量
					innerObject.put("unit_zh", stockPositionVo.getNameZh());// 单位
...
//						item.put("unit_name", vo.getNameZh());
//						bizStockTaskItemAry.add(item);
//					}
...
//	}
	...
	public @ResponseBody JSONObject submit(@RequestBody JSONObject json, CurrentUser cUser) {
		...
		try {
			HashMap<String, Object> returnValue = taskServiceImpl.addBizStockTaskReqHead(json);
			if (returnValue != null) {
	...
	// 仓位的物料信息
	@RequestMapping(value = "/warehouse/add_pkg_operation", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	public @ResponseBody JSONObject addPkgOperation(@RequestBody JSONObject json) {
		int error_code = EnumErrorCode.ERROR_CODE_FAILURE.getValue();
		boolean returnStatus = false;
		JSONArray body = new JSONArray();
		BizPkgOperationHead head = new BizPkgOperationHead();
		List<BizPkgOperationItem> items = new ArrayList<BizPkgOperationItem>();
		try {
			head.setPkgOperationId(2);
			head.setPkgOperationCode(commonService.getNextReceiptCode("pkg_operation"));
			head.setReferReceiptId(1);
			head.setReferReceiptType((byte) 61);
			head.setCreateUser("a123457");
			head.setIsDelete((byte) 0);
			head.setCreateTime(new Date());
			head.setModifyTime(new Date());

			for (int i = 1; i < 3; i++) {
				BizPkgOperationItem item = new BizPkgOperationItem();
				item.setPkgOperationRid(i);
				//item.setPalletId(1);
				//item.setPackageId(1);
				item.setMatId(1);
				item.setBatch(Long.valueOf("1000000297"));
				//item.setSnId(i);
				//item.setSourcePalletId(5);
				//item.setSourcePackageId(5);
				item.setIsDelete((byte) 0);
				item.setCreateTime(new Date());
				item.setModifyTime(new Date());
				items.add(item);
			}
			//packageService.insertPkgOperationHeadAndItems(head, items);
			returnStatus = true;
			error_code = EnumErrorCode.ERROR_CODE_SUCESS.getValue();
		} catch (Exception e) {
			logger.error("添加打包单", e);
		}
		return UtilResult.getResultToJson(returnStatus, error_code, body);
	}
