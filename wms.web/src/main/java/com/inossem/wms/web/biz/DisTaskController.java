...
public class DisTaskController {
	...
	public @ResponseBody JSONObject bizStockTaskReqHeadList(CurrentUser cUser, @RequestBody JSONObject json) {
		...
		try {
			...
			int chooseType = json.getInt("choose_type");// 1未完成下架请求 2已完成下架请求
														// 3未入库下架作业 4已入库下架作业
	...
	public @ResponseBody JSONObject checkCodeType(@RequestBody JSONObject json, CurrentUser cUser) {
		...
		try {
			if(json.containsKey("refer_receipt_type")){
				...
				if(referReceipt.equals(EnumReceiptType.STOCK_TRANSPORT_PRODUCTION_URGENT.getValue())||referReceipt.equals(EnumReceiptType.STOCK_OUTPUT_PRODUCTION_URGENT_FALSE.getValue())){
					json.put("isUrgent", 1);
				}
			}
	...
	@RequestMapping(value = {
			"/removal/area_list" }, method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	public @ResponseBody JSONObject getAreaList(@RequestBody JSONObject json, CurrentUser cUser) {
		...
		try {
			...
			body = taskServiceImpl.getAreaListByRemoval(map);
	...
	// 上架作业--上架
	@RequestMapping(value = "/removal/save_remark", method = RequestMethod.PUT, produces = "application/json;charset=UTF-8")
	...
	@RequestMapping(value = "/removal/delete/{stock_task_id}", method = RequestMethod.DELETE, produces = "application/json;charset=UTF-8")
