 ...
 */
@Controller
@RequestMapping("/biz/input/account/production")
public class InputAccountProduceController {
	
	private static final Logger logger = LoggerFactory.getLogger(InputAccountProduceController.class);
	 

	@Autowired
	private IInputAccountProduceService iInputProduceTransportService;
...
//					Map<String,Object> unit_map=iInputProduceService.getUnitIdByCode(order_map.get("GMEIN").toString().trim());
	...
	public JSONObject production_input_list(CurrentUser cuser, @RequestBody JSONObject json) {
		...
		try {
			...
			map.put("receiptType", EnumReceiptType.STOCK_INPUT_BOOK_URGENT.getValue());
