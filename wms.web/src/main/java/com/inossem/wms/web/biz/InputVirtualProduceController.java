 ...
 */
@Controller
@RequestMapping("/biz/input/virtual/production")
public class InputVirtualProduceController {
	
	private static final Logger logger = LoggerFactory.getLogger(InputVirtualProduceController.class);
	 

	@Autowired
	private IInputVirtualProduceService iInputProduceTransportService;
	@Autowired
...
//					List<Map<String,Object>> erp_batch_list=iInputProduceService.selectErpList(mat_id,dicFactory.getFtyId());
					...
					map.put("syn_type_id", EnumSynType.SAP_SYN.getValue());
...
//					Map<String,Object> unit_map=iInputProduceService.getUnitIdByCode(order_map.get("GMEIN").toString().trim());
	...
	public JSONObject production_input_list(CurrentUser cuser, @RequestBody JSONObject json) {
		...
		try {
			...
			map.put("receiptType", EnumReceiptType.STOCK_INPUT_VIRTUAL.getValue());
...
//			}
			...
			List<String> transportStatus=iInputProduceTransportService.selectTranspotStatus(stock_input_id);
			if(transportStatus.contains("10") || transportStatus.contains("0")) {
				result.put("transport_status", "1");
			}else {
				result.put("transport_status", "0");
			}
...
//			ErpReturn er=iInputProduceService.takeSapWriteOff(item_id,doc_year);
