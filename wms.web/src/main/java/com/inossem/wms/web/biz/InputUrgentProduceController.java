 ...
 */
@Controller
@RequestMapping("/biz/input/urgent/production")
public class InputUrgentProduceController {
	
	private static final Logger logger = LoggerFactory.getLogger(InputUrgentProduceController.class);
	 
	@Autowired
	private CwMesWsImpl iCwMesWs;
	@Autowired
	private IInputUrgentProduceService iInputProduceTransportService;
...
//					List<Map<String,Object>> erp_batch_list=iInputProduceService.selectErpList(mat_id,dicFactory.getFtyId());
					List<Map<String,Object>> packageTypelist=iInputProduceService.selectPackageTypeList(mat_id,dicFactory.getFtyId());		
...
//					map.put("erp_batch_list", erp_batch_list);				
					...
					map.put("unit_name", order_map.get("GMEIN"));					
	...
	@RequestMapping(value="/to_urgent_produce_add",method = RequestMethod.GET, produces = "application/json;charset=UTF-8")
	@ResponseBody
	public JSONObject func6(CurrentUser cuser) {
		String msg="";
		Map<String, Object> map = new HashMap<String, Object>();
		...
		try {
				...
				List<Map<String,Object>> erp_batch_list=iInputProduceTransportService.selectAllErpList();
				List<Map<String,Object>> packageTypelist=iInputProduceTransportService.selectAllPackageTypeList();		
				List<Map<String,Object>> classTypeList=inputTransportService.selectAllclassType();
				List<Map<String,Object>> factoryList=iInputProduceTransportService.selectFactoryList(cuser);
				...
				map.put("class_type_list", classTypeList);
				map.put("erp_batch_list", new ArrayList<Map<String,Object>>() );				
				map.put("package_type_list", new ArrayList<Map<String,Object>>());
				map.put("fty_list", factoryList);				
...
//				DicFactory dicFactory=iInputProduceTransportService.selectFtyNameById(order_map.get("WERKS").toString());
//		
//				List<Map<String,Object>> locationlist=iInputProduceTransportService.selectLocationList(dicFactory.getFtyId());
//				map.put("location_list", locationlist);									
				map.put("create_user", cuser.getDisplayUsername());
				SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");  
				java.util.Date date=new java.util.Date();  				
	...
	@RequestMapping(value="/select_mat_info",method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	@ResponseBody
	public JSONObject select_mat_info(CurrentUser cuser,@RequestBody JSONObject json) {
		String msg="";
		Map<String, Object> map = new HashMap<String, Object>();
		...
		try {
				...
				int fty_id=json.getInt("fty_id");
				Map<String,Object> mat_map=iInputProduceTransportService.select_mat_info(mat_code);
				if(mat_map!=null) {
					map.put("mat_map", mat_map);
//					List<Map<String,Object>> erp_batch_list=iInputProduceService.selectErpList(mat_map.get("mat_id").toString(),fty_id);
//					if(erp_batch_list.size()==0) {
//						map.put("msg", "未找到该物料对应的erp批次");
//					}
					List<Map<String,Object>> packageTypelist=iInputProduceService.selectPackageTypeList(mat_map.get("mat_id").toString(),fty_id);	
					if(packageTypelist.size()==0) {
						map.put("msg", "未找到该物料对应的包装类型");
					}
//					map.put("erp_batch_list", erp_batch_list);				
					map.put("package_type_list", packageTypelist);
				}else {
...
//	@RequestMapping(value="/select_location_info/{fty_id}",method = RequestMethod.GET, produces = "application/json;charset=UTF-8")
//	@ResponseBody
//	public JSONObject select_location_info(@PathVariable("fty_id") int fty_id,CurrentUser cuser) {
//		String msg="";
//		Map<String, Object> map = new HashMap<String, Object>();
//	
//		boolean status = true;
//		Integer errorCode = EnumErrorCode.ERROR_CODE_SUCESS.getValue();		
//		try {
//			List<Map<String,Object>> locationlist=iInputProduceTransportService.selectLocationList(fty_id);
//			map.put("location_list", locationlist);													
//		}catch (Exception e) {
//			logger.error("新增生产入库", e);
//			errorCode = EnumErrorCode.ERROR_CODE_EXCEPTION.getValue();
...
//	}
	...
	public JSONObject production_input_list(CurrentUser cuser, @RequestBody JSONObject json) {
		...
		try {
			...
			map.put("receiptType", EnumReceiptType.STOCK_INPUT_PRODUCTION_URGENT.getValue());
...
//				if("S".equals(er.getType())) {
					iInputProduceTransportService.modifyNum(stockInputHead,user,json,batchArray);	
...
//					iInputProduceTransportService.updateheadStatus(bizStockInputHead);				
//				}
				status=true;	
			}else {
				...
				stockInputHead=(BizStockInputHead)map.get("bizStockInputHead");
				stockInputHead.setPurchaseOrderCode(json.get("produce_order_code").toString());
...
//				}
	...
	public @ResponseBody Object getMaterialBatchInfo(@RequestBody JSONObject json,CurrentUser cuser) {
		...
		try {
			...
			String stock_input_id=json.get("stock_input_id").toString();
			...
			if(er.getType().equals("S")) {
				iInputProduceTransportService.updateStockToWriteOff(stock_input,cuser);
				iInputProduceTransportService.updateWriteOffStatus(er,item_id,stock_input);
			}else {
				errorString="冲销失败";
			}			
		} catch (Exception e) {
	...
	public JSONObject delete(@PathVariable("stock_input_id") String stock_input_id,CurrentUser user) {
		...
		int error_code = EnumErrorCode.ERROR_CODE_FAILURE.getValue();
		String msg = EnumErrorCode.getNameByValue(error_code);
		Map<String, Object> result = new HashMap<String, Object>();
		try {
			iInputProduceTransportService.delete(stock_input_id,user);	
			error_code = EnumErrorCode.ERROR_CODE_SUCESS.getValue();
			msg = EnumErrorCode.getNameByValue(error_code);
		} catch (WMSException e) {
			logger.error("生产单信息", e);
			msg = e.getMessage();
			status = false;
