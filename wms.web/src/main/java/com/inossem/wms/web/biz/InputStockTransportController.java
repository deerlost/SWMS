...
@Controller
@RequestMapping("/biz/input/transport")
public class InputStockTransportController {

	private static final Logger logger = LoggerFactory.getLogger(InputStockTransportController.class);
	...
	public @ResponseBody Object platformBaseInfo(CurrentUser cUser) {
		...
		try {					
			...
			jsonObject2.put("syn_type_name", EnumSynType.MES_SYN.getName());
			jsonObject2.put("syn_type_id", EnumSynType.MES_SYN.getValue());
	...
	public @ResponseBody Object takeMes(CurrentUser user,@PathVariable("stock_input_id") String stock_input_id) {
		...
		try {
			...
			if(bizStockInputTransportHead.getStatus().toString().equals("20")) {
				String result=inputTransportService.takeMesFailAgain(bizStockInputTransportHead,user);
			}else {
				WmIopRetVal wmIopRetVal=inputTransportService.takeMesAgain(bizStockInputTransportHead,user);
				if(!"".equals(wmIopRetVal.getErrmsg()) && wmIopRetVal.getErrmsg()!=null){
...
//					inputTransportService.updateMatDocCode(er,bizStockInputTransportHead);					
					...
					try {
						if(!bizStockInputTransportHead.getSynType().toString().equals(String.valueOf(EnumSynType.SAP_SYN.getValue()))) {
...
//				}
	...
	public @ResponseBody Object getMoveReasonList(String  task_id,String receipt_type,String time) {
		...
		try {
			...
			map.put("time", UtilString.getStringOrEmptyForObject(time).trim());
	...
	public @ResponseBody Object getTransportInputList(CurrentUser cuser, @RequestBody JSONObject json) {
		...
		try {
			...
			map.put("receiptType", EnumReceiptType.STOCK_INPUT_TRANSPORT_ERP.getValue());
...
//			
	...
	public @ResponseBody Object getInputOtherInfo(CurrentUser cuser,@PathVariable("stock_input_id") Integer stock_input_id
		...
		Integer errorCode = EnumErrorCode.ERROR_CODE_SUCESS.getValue();
		String msg = EnumErrorCode.ERROR_CODE_FAILURE.getName();
		boolean status = true;
...
//			}
			if(itemList.get(0).get("business_type").toString().equals(EnumReceiptType.STOCK_TRANSPORT_FACTORY.getValue().toString())
					||itemList.get(0).get("business_type").toString().equals(EnumReceiptType.STOCK_TRANSPORT_STOCK.getValue().toString())
					||itemList.get(0).get("business_type").toString().equals(EnumReceiptType.STOCK_TRANSPORT_MATERIAL.getValue().toString())){
				input.put("write_off", 0);
			}else {
				input.put("write_off", 1);
			}
			if(!headMap.get("status").toString().equals("10")) {
				input.put("write_off", 1);
			}
			msg = EnumErrorCode.ERROR_CODE_SUCESS.getName();
		} catch (Exception e) {
			status = false;
			errorCode = EnumErrorCode.ERROR_CODE_EXCEPTION.getValue();
	...
	public @ResponseBody Object write_off(@RequestBody JSONObject json,CurrentUser cuser) {
		...
		try {
			...
			bizStockInputTransportHead.setStockTransportId(Integer.valueOf(json.get("stock_task_id").toString()));
			bizStockInputTransportHead=inputTransportService.selectEntity(bizStockInputTransportHead);
			List<Map<String,Object>> list=inputTransportService.selectItemList(Integer.valueOf(json.get("stock_task_id").toString()));
			List<Map<String,Object>> itemList = inputTransportService.getMatListByStockID(list);		
//			ErpReturn er=inputTransportService.takeSapWriteOff(bizStockInputTransportHead,itemList);
//			if("S".equals(er.getType())) {
			String flag=inputTransportService.selectIsWriteOff(Integer.valueOf(json.get("stock_task_id").toString()));
			if(flag.equals("0")) {			
				inputTransportService.updateStockToWriteOff(Integer.valueOf(json.get("stock_task_id").toString()),cuser,itemList,bizStockInputTransportHead);
				try {
					if(!bizStockInputTransportHead.getSynType().toString().equals(String.valueOf(EnumSynType.SAP_SYN.getValue()))) {	
						String result=inputTransportService.takeMesFail(bizStockInputTransportHead,itemList,cuser);
						if(result==null) {
							errorString=result;
						}
					}
