...
function dataBind() {
	$("#id_div_grid").iGrid({
		columns: [ 
			{ th: '仓库号', col: 'wh_code', min: 60 },
			{ th: '仓库描述', col: 'wh_name', min: 60 },
			{ th: '仓库地址', col: 'address' },
			{ th: '操作', col: 'wh_code', class:"", min:60,align: 'left',type:"link",sort:false,
			link:{
					display:"修改",
					href:"editck.html?action=modify",//默认带index= 数据row下标。
					args:['wh_id']//还需要什么参数，都写在这里，如例子：合同编号、采购订单号
				}
			},
		], 
		data: indexdata.body,
...
function search(page) {
	var haspage = page || -1;
	showloading();
	var url = uriapi + '/conf/wh/warehouse_list';
	var jsondata = {
