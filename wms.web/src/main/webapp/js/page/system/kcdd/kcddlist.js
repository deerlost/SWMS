...
function dataBind() {
	$("#id_div_grid").iGrid({
		columns: [ 
			{ th: '库存地点', col: 'location_code' },
			{ th: '库存地点描述', col: 'location_name' },
			{ th: '库存地点地址', col: 'address_location' },
			{ th: '工厂', col: 'fty_code' },
			{ th: '工厂名称', col: 'fty_name' },
			{ th: '工厂地址', col: 'address_factory' },
			{ th: '创建日期', col: 'create_time' },
			{ th: '所属公司', col: 'corp_code' },
			{ th: '公司名称', col: 'corp_name' },
			{ th: '操作', col: 'location_id', class:"", sort:false, min:60,align: 'left',type:"popup",
			popup:{
					display:"修改",
					href:"editkcdd.html?action=modify",//默认带index= 数据row下标。
					area:"700px,550px",
					title:"修改",
					args:['location_id','fty_id']//还需要什么参数，都写在这里，如例子：合同编号、采购订单号
				}
			},
		], 
		data: indexdata.body,
...
function search(page) {
	var haspage = page || -1;
	showloading();
	var url = uriapi + '/conf/loc/list_loc';
	var jsondata = {
