...
function init(){
    showloading();
    var url=uriapi+"/biz/task/shelves/biz_stock_task_req_details/"+GetQueryString("id")
    ...
    $.each(data,function(i,item){
        if(item.qty!="" && item.qty!="0"){
            pallet_package_list.push({
                ...
                target_position_id:item.position_id,
                qty:item.qty
            })
        }
    })
...
function dataBindTable(){
    g1=$("#id_div_grid").iGrid({
        columns: [
            ...
            { th: '未上架数量', col: 'unstock_task_qty', sort:false,type:'number'},
            { th: '本次上架数量', col: 'qty', sort:false,class:'qty',type:'number'},
...
function submit(){
    ...
    var url=uriapi+"/biz/task/shelves/submit"
    showloading();
