...
function init(){
    showloading();
    var url=uriapi+"/biz/task/removal/get_inner_task_details/"+GetQueryString("id")
...
function dataBindTable(){
    g1=$("#id_div_grid").iGrid({
        columns: [
            ...
            { th: '未下架数量', col: 'unstock_task_qty', sort:false,width:100 ,type:'number'},
            { th: '本次下架数量', col: 'qty', sort:false,width:150 ,type:'number'},
...
function submit(){
    var item_list=[]
    var url=uriapi+"/biz/task/removal/save_remark"
    $.each(indexdata.item_list,function(i,item){
        item_list.push({
            remark:item.remark,
...
function clickEvent(){
    ...
    $(".btndelete").click(function() {
        layer.confirm("是否删除", {
            ...
            var url=uriapi+"/biz/task/removal/delete/"+GetQueryString("id")
            showloading();
...
function print(){
    showloading()
    var url=uriapi+"/biz/print/print_down_task/"+GetQueryString("id")
    ajax(url,"get",null,function(data){
        if(data.head.status==true){
            layer.close(indexloading)
