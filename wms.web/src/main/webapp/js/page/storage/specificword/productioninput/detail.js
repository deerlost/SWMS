...
function init(){
    showloading();
    var url=uriapi+"/biz/input/urgent/production/production_input_info/"+GetQueryString("id")
...
function getSelectData(){
    showloading()
    var url=uriapi+"/biz/input/urgent/production/to_urgent_produce_add"
    ajax(url,"get",null,function(data){
        if(data.head.status==true){
            layer.close(indexloading);
...
//加载物料信息
function getMat(){
    var mat_code=$(".mat_code").val()
    if(mat_code=="") return false
    var obj={
        mat_code:mat_code,
        fty_id:$(".fty_name").val()
    }
    var url=uriapi+"/biz/input/urgent/production/select_mat_info"
    ajax(url,"post",JSON.stringify(obj),function(data){
        if(data.head.status==true){
            $(".disabled").removeClass("disabled")
            indexdata.mat_code=data.body.mat_map.mat_code
            indexdata.mat_name=data.body.mat_map.mat_name
            indexdata.mat_id=data.body.mat_map.mat_id
            indexdata.unit_name=data.body.mat_map.unit_name
            indexdata.unit_id=data.body.mat_map.unit_id
...
function search(){
    ...
    var url=uriapi+"/biz/input/urgent/production/contract_info/"+$(".search").val()
    showloading();
    ajax(url,"get",null,function(data){
        if(data.head.status==true){
        	var count=0
            ...
            if(indexdata.status==2){
                var is_checked=true
                if(data.body.mat_code!=indexdata.mat_code){
                    is_checked=false
                }
                if(data.body.fty_name!=indexdata.fty_name){
                    is_checked=false
                }
                $.each(indexdata.item_list,function(i,item){
                    count=countData.add(count,item.input_stock_num)
                })
                if(data.body.input_stock_num<count){
                    is_checked=false
                }
                if(is_checked==false){
                    layer.msg("生产订单号不匹配")
                    ...
                    indexdata.produce_order_code=data.body.produce_order_code
                    $("[c-model='produce_order_code']").html(indexdata.produce_order_code)
                ...
                $(".fty_name").parent().html("<span c-model='fty_name'></span>")
                $(".input_stock_num").parent().html("<span c-model='input_stock_num'></span>")
                $.each(indexdata.package_type_list,function(i,item){
                    $.each(item.erp_batch_list,function(j,itemer){
                        indexdata.erp_batch_list.push({
                            erp_batch_code:itemer.erp_batch_code,
...
//绑定物料信息
function dataBindMat(){
    $("[c-model='mat_name']").html(indexdata.mat_name)
    $("[c-model='unit_name']").html(indexdata.unit_name)
}
...
function dataBindSelect(){
    ...
    $.each(selectdata.syn_type_list,function(i,item){
        $(".syn_type_name").append("<option value='"+item.syn_type_id+"'>"+item.syn_type_name+"</option>")
    })
    $.each(selectdata.fty_list,function(i,item){
        $(".fty_name").append("<option value='"+item.fty_id+"' data-index='"+i+"' data-code='"+item.fty_code+"'>"+item.fty_name+"</option>")
    })

    $(".class_type_id").val(selectdata.class_type_id)
    $(".syn_type_name").val(selectdata.syn_type_id)
...
function dataBindBase() {
    ...
    if(indexdata.status!=null){
        ...
        }else if(indexdata.status==2 || indexdata.status==0){
            $(".btn_submit").html("　过帐　")
            $(".btnoperate").show()
        }
        $.each(indexdata.item_list,function(i,item){
...
function clickEvent(){
    ...
    $(".btndelete").click(function() {
        layer.confirm("是否删除", {
            ...
            layer.close(parseInt($(".layui-layer-shade").attr("times")));
            var url=uriapi+"/biz/input/urgent/production/delete/"+GetQueryString("id")
            showloading();
            ajax(url,"get",null,function(data){
                if(data.head.status==true){
...
function AddMaterial(){
    ...
    var index=$(".fty_name option:selected").attr("data-index")
    if(index){
        indexdata.item_list.push({
            ...
            package_num:"--",
            location_id:selectdata.fty_list[index].location_list[0].location_id,
            package_standard:indexdata.package_type_list[0].package_standard,
            pack_list:[],
            name_zh:indexdata.package_type_list[0].name_zh,
            decimal_place:0,
            location_list:selectdata.fty_list[index].location_list,
            package_type_list:indexdata.package_type_list,
            erp_batch_list:indexdata.erp_batch_list,
            input_stock_num:"添加",
            rec_package_id:"添加",
            mes_list:[]
        })
        ...
        indexdata.item_list.push({
            ...
            pack_list:[],
            decimal_place:0,
            location_list:indexdata.location_list,
            package_type_list:indexdata.package_type_list,
            erp_batch_list:indexdata.erp_batch_list,
            input_stock_num:"添加",
            mat_id:indexdata.mat_id,
...
function getMES(index){
    if(indexdata.fty_code==undefined){
        return [$(".fty_name option:selected").attr("data-code"),$(".product_line_id option:selected").html(),indexdata.item_list[index].mes_list,indexdata.item_list[index],indexdata.mat_code,indexdata.unit_name]
    }else{
...
function dataBindTable(){
    g=$("#id_div_grid").iGrid({
        columns: [
            { th: '包装类型', col: 'package_type_id',sort:false,width:110,type:table.select,data:'package_type_list',value:"package_type_id",text:"package_type_code"},
            { th: '包装规格', col: 'package_standard_ch',sort:false,width:110},
            { th: '件数', col: 'package_num',sort:false,width:150},
            { th: '入库数量', col: 'input_stock_num',sort:false,width:150,type:'button',class:'input_stock',button:{
            ...
            { th: '库存地点', col: 'location_id',sort:false,type:table.select,data:'location_list',value:'location_id',text:["location_code","location_name"],width:230,class:'location_id'},
            { th: '关联包装单（KG）', col: 'rec_package_id',sort:false,width:"140",type:"popup",class:'rec_package_id',
                popup:{
        ...
        loadcomplete:function(){
            ...
            if(indexdata.item_list[indexdata.item_list.length-1].location_code==undefined)
                indexdata.item_list[indexdata.item_list.length-1].location_code=indexdata.item_list[0].location_list[0].location_code;
        },
...
function submit(){
    ...
    if(indexdata.status==2 && indexdata.produce_order_code==""){
        layer.msg("请输入生产订单号")
        return false
    }
    if($(".input_stock_num").val()==""){
        layersMoretips("必填项",$(".input_stock_num"))
        return false
    }
    ...
    $.each(indexdata.item_list,function(i,item){
        ...
        if(item.input_stock_num>0){
            ...
            if(item.sn_used==1 && item.input_stock_num!=item.standard_num){
                layersMoretips("入库数量与包装单数量不一致",$(".input_stock").eq(i))
                is_checked=false
            }
            ...
            if(item.package_standard_ch!=1 && countData.mod(item.input_stock_num,item.package_standard)!=0){
                layersMoretips("入库数量为包装规格的整数倍",$(".input_stock").eq(i))
                is_checked=false
            }
    ...
    if(!CheckisDecimal($(".input_stock_num")) && $(".input_stock_num").val()!=undefined){
        is_checked=false
    }else/*{
    	if($("[c-model='input_stock_num']").html()==undefined){
            if(parseFloat(num)>$(".input_stock_num").val()){
    ...
    if(indexdata.produce_order_code==undefined){
        obj.fty_id=$(".fty_name").val()
        obj.input_stock_num=$(".input_stock_num").val()
    }


    var url=uriapi+"/biz/input/urgent/production/production"
    showloading();
    ajax(url,"post",JSON.stringify(obj),function(data){
        closeTipCancel()
...
function writeOff(){
    ...
    var url=uriapi+"/biz/input/urgent/production/write_off"
    showloading()
    ajax(url,"post",JSON.stringify(obj),function(data){
        if(data.head.status==true){
...
//mes同步
function submitMES(){
    var url=uriapi+"/biz/input/urgent/production/takeMes/"+GetQueryString("id")
    showloading()
    ajax(url,"get",null,function(data){
        if(data.head.status==true){
            closeTipCancel()
...
$(function () {
    ...
    $("a.produce").click(function () {
        search();
    });
    $("input.mat_code").keyup(function (e) {
        var keycode = e.which;
        if (keycode == 13) {
            getMat();
        }
    });
    $("a.btn-mat_code").click(function () {
        getMat();
    });
    ...
    $(".fty_name").change(function () {
        indexdata.item_list=[]
/*        var index = $(".fty_name option:selected").attr("data-index")
        $.each(indexdata.item_list, function (i, item) {
            item.location_list = selectdata.fty_list[index].location_list
            item.location_id = selectdata.fty_list[index].location_list[0].location_id
        })*/
        dataBindTable()
    })
