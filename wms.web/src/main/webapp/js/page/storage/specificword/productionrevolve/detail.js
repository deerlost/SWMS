var indexdata={},selectdata={},g=null,table={select:"select",popup:"",checkbox:true}
...
function getBaseInfo(){
    ...
    if(GetQueryString("id")==null){
        indexdata={}
        indexdata.item_list=[]
        var url=uriapi+"/biz/urgent/product/transport/base_info"
        ...
        var url=uriapi+"/biz/urgent/product/transport/details/"+GetQueryString("id")
        ajax(url,"get",null,function(data){
            if(data.head.status==true){
                layer.close(indexloading);
                indexdata=data.body
...
function dataBindTable(){
    g=$("#id_div_grid").iGrid({
        columns: [
            ...
            { th: '单位', col: 'package_type_name',sort:false},
...
function clickEvent(){
    ...
    $(".btndelete").click(function() {
        layer.confirm("是否删除", {
            ...
            var url=uriapi+"/biz/urgent/product/transport/delete/"+GetQueryString("id")
            showloading();
...
function submit(){
    ...
    $.each(indexdata.item_list,function(i,item){
        ...
        if(item.transport_qty!="" && item.transport_qty!=0){
            ...
            if($(".txttransport_qty[data-index-row='"+i+"']").length!=0){
                ...
                if(item.package_standard_ch!=1 && countData.mod($(".txttransport_qty[data-index-row='"+i+"']").val(),item.package_standard)!=0){
    ...
    var url=uriapi+"/biz/urgent/product/transport/submit"
