...
function dataBind() {
	$("#id_div_grid").iGrid({
		columns: [ 
			{ th: 'id', col: 'id', min: 60 },
			{ th: '接收方编码', col: 'receive_code', min: 60 },
			{ th: '接收方描述', col: 'receive_name' },
			{ th: '操作', col: 'id', class:"", min:60,align: 'left',type:"popup",
				popup:{
					display:"修改",
					href:"addreceiver.html",//默认带index= 数据row下标。
					area:"500px,350px",
					title:"修改",
					args:['id']//还需要什么参数，都写在这里，如例子：合同编号、采购订单号
				}
			},
		], 
		data: indexdata.body,
...
function search(page) {
	var haspage = page || -1;
	showloading();
	var url = uriapi + '/conf/receiver/list_receiver';
	var jsondata = {
