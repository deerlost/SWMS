...
function init(){
    ...
    showloading();
    var url=uriapi+"/biz/input/account/production/production_input_info/"+GetQueryString("id")
...
function search(){
    ...
    var url=uriapi+"/biz/input/account/production/contract_info/"+$(".search").val()
    showloading();
    ajax(url,"get",null,function(data){
        if(data.head.status==true){
            layer.close(indexloading);
...
function clickEvent(){
    ...
    $(".btndelete").click(function() {
        layer.confirm("是否删除", {
            ...
            layer.close(parseInt($(".layui-layer-shade").attr("times")));
            var url=uriapi+"/biz/input/account/production/delete/"+GetQueryString("id")
            showloading();
            ajax(url,"get",null,function(data){
                if(data.head.status==true){
...
/*            { th: '关联包装单', col: 'rec_package_id',sort:false,width:"120",type:"popup",class:'rec_package_id',
                popup:{
                    display:table.popupmsg,
...
/*            if(position_list.length==0 && GetQueryString("id")==null){
    ...
    var url=uriapi+"/biz/input/account/production/production"
    showloading();
...
function writeOff(check){
    ...
    var url=uriapi+"/biz/input/account/production/write_off"
    showloading()
    ajax(url,"post",JSON.stringify(obj),function(data){
        if(data.head.status==true){
